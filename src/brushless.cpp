...
void brushless::startup() {
	...
	while ((timer1_pwm->getFrequency() < freqData->end)) {
		...
		if (timer1_pwm->getFrequency() < freqData->end) {
			...
			timer1_pwm->setFrequency(
					getStartupValueHz(freqData->gain, freqData->start));
		}
...
// Debug print
	case 'o':
		return "GAIN:" + String(freqData->gain) + " END:"
				+ String(freqData->end);

// Formatted print for parsing
	case 'p':
		return String("\n--QUERY--\n") + String("f_t1 ")
				+ String(timer1_pwm->getFrequency()) + String(" Hz\n")
				+ String("TOP_t1 ") + String(timer1_pwm->getTop())
				+ String("\n") + String("d_t1 ") + String(timer1_pwm->getDuty())
				+ String("\n") + String("f_t0 ")
				+ String(timer0_pwm->getFrequency()) + String("\n")
...
String brushless::angSpeed() {
	...
	return "RPM elettrici:" + String(RPM_e) + ", RPM meccanici:" + String(RPM_m)
			+ ", RAD/s elettrici:" + String(rads_e) + ", RAD/s meccanici:"
			+ String(rads_m);
}
