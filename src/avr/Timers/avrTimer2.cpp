...
void
timer2_init (timer_mode _mode, uint16_t _prescaler)
{
...
void
timer2_mode_phasecorrect_top_init ()
{
}
void
timer2_mode_fastpwm_top_init ()
{
}
void
timer2_mode_fastpwm_ocra_init ()
...
void
timer2_setPin (char _pin, timer_pin_mode _mode)
{
...
void
_timer2_setPrescaler (uint16_t _prescaler)
{
...
void
timer2_setPrescaler (uint16_t _prescaler)
{
  ...
  for (i = 0; i < timer2_allowedPrescalersLenght; i++)
    {
      if (timer2_prescaler == timer2_allowedPrescalers[i])
        {
          return timer2_allowedPrescalers[(i - 1)
                                          % timer2_allowedPrescalersLenght];
        }
    }
}

uint16_t
timer2_getNextPrescaler ()
{
...
uint16_t
timer2_getRequiredPrescaler (uint32_t Hz)
{

  uint8_t i;
  for (i = 0; i < timer2_allowedPrescalersLenght; i++)
    {
      if (Hz >= prescalerMinHz (timer2_mode, timer2_allowedPrescalers[i],
                                TIMER_REGISTER_SIZE))
        {
...
timer2_resetCounter ()
{
  ...
  if (timer2_mode == CTC_OCRA || timer2_mode == FASTPWM_OCRA
      || timer2_mode == PHASE_CORRECT_OCRA)
    {
      TCNT2 = 0;
    }
}
...
timer2_start ()
{
  timer2_start (timer2_prescaler);
}

void
timer2_start (uint16_t _prescaler)
{
  if (timer2_mode == NORMAL)
    {
      timer2_resetCounter ();
...
timer2_setTop (uint16_t _top)
{
  ...
  if (timer2_mode == PHASE_CORRECT_OCRA || timer2_mode == CTC_OCRA
      || timer2_mode == FASTPWM_OCRA)
    {
...
void
timer2_setBottom (uint8_t _bottom)
{
  timer2_bottom = _bottom;
}

void
timer2_setFrequency (uint32_t Hz)
{
...
timer2_setDuty (uint8_t _duty)
{
  ...
  if (timer2_mode == PHASE_CORRECT_OCRA || timer2_mode == FASTPWM_OCRA
      || timer2_mode == CTC_OCRA)
    {
...
uint16_t
timer2_getPrescaler (void)
{
  return timer2_prescaler;
}

uint32_t
timer2_getFrequency (void)
{
  if (timer2_mode == PHASE_CORRECT_OCRA || timer2_mode == PHASE_CORRECT_TOP)
    {
...
void
timer2_register_COMPA_callback (void (*func) (void))
{
  SET_TIMER2_INTERRUPT_OUTPUTCOMPARE_A;
  timer2_compa_handler = func;
}
void
timer2_register_COMPB_callback (void (*func) (void))
{
  SET_TIMER2_INTERRUPT_OUTPUTCOMPARE_B;
  timer2_compb_handler = func;
}
void
timer2_register_OVF_callback (void (*func) (void))
{
  SET_TIMER2_INTERRUPT_OVERFLOW;
