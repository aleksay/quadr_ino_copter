...
uint8_t timer1_allowedPrescalersLenght
    = sizeof (timer1_allowedPrescalers) / sizeof (timer1_allowedPrescalers[0]);
...
void
timer1_init (timer_mode _mode, uint16_t _prescaler)
{
...
void
timer1_mode_phasecorrect_top_init ()
{
  UNSET_TIMER1_PINA;
  UNSET_TIMER1_PINB;
  TIMER1_RESET;
...
void
timer1_setPin (char _pin, timer_pin_mode _mode)
{
...
void
_timer1_setPrescaler (uint16_t _prescaler)
{
  switch (_prescaler)
    {
    case 0:
...
void
timer1_setPrescaler (uint16_t _prescaler)
{
  ...
  for (i = 0; i < timer1_allowedPrescalersLenght; i++)
    {
      if (timer1_prescaler == timer1_allowedPrescalers[i])
        {
          return timer1_allowedPrescalers[(i - 1)
                                          % timer1_allowedPrescalersLenght];
        }
    }
}

uint16_t
timer1_getNextPrescaler ()
{
...
void
timer1_increasePrescaler ()
{
  timer1_setPrescaler (timer1_getNextPrescaler ());
}
void
timer1_decreasePrescaler ()
{
  timer1_setPrescaler (timer1_getPrevPrescaler ());
}

uint16_t
timer1_getRequiredPrescaler (uint32_t Hz)
{

  uint8_t i;
  for (i = 0; i < timer1_allowedPrescalersLenght; i++)
    {
      if (Hz >= prescalerMinHz (timer1_mode, timer1_allowedPrescalers[i],
                                TIMER_REGISTER_SIZE))
        {
...
timer1_resetCounter ()
{
  ...
  if (timer1_mode == NORMAL || timer1_mode == FASTPWM_TOP
      || timer1_mode == PHASE_CORRECT_TOP)
    {
      cli ();
      TCNT1 = timer1_bottom;
      sei ();
    }
  if (timer1_mode == CTC_OCRA || timer1_mode == FASTPWM_OCRA
      || timer1_mode == PHASE_CORRECT_OCRA
      || timer1_mode == PHASEFREQ_CORRECT_OCRA)
    {
      cli ();
      TCNT1 = 0;
      sei ();
    }
  if (timer1_mode == PHASEFREQ_CORRECT_ICR || timer1_mode == PHASE_CORRECT_ICR
      || timer1_mode == CTC_ICR || timer1_mode == FASTPWM_ICR)
    {
      cli ();
      TCNT1 = 0;
      sei ();
...
timer1_start ()
{
  timer1_start (timer1_prescaler);
}

void
timer1_start (uint16_t _prescaler)
{
  // timer1_resetCounter();
...
void
timer1_setFrequency (uint32_t Hz)
{
  /*
    int8_t ret;
   ...
   // }
    // debug("Hz:%u, Required prescaler:%u", Hz,
   timer1_getPrescalerRequired(Hz));
    ...
    return 0;
    */
  if (timer1_mode == FASTPWM_OCRA || timer1_mode == FASTPWM_ICR
      || timer1_mode == FASTPWM_TOP)
    {
...
timer1_setTop (uint16_t _top)
{
  ...
  if (timer1_mode == PHASE_CORRECT_OCRA || timer1_mode == CTC_OCRA
      || timer1_mode == FASTPWM_OCRA)
    {
...
void
timer1_setBottom (uint8_t _bottom)
{
  timer1_bottom = _bottom;
}

void
timer1_setDuty (uint8_t _duty)
{
  ...
  if (timer1_mode == PHASE_CORRECT_OCRA || timer1_mode == FASTPWM_OCRA
      || timer1_mode == CTC_OCRA)
    {
...
uint16_t
timer1_getPrescaler (void)
{
  return timer1_prescaler;
}

uint32_t
timer1_getFrequency (void)
{
  if (timer1_mode == FASTPWM_OCRA || timer1_mode == FASTPWM_ICR
      || timer1_mode == FASTPWM_TOP)
    {
...
void
timer1_timer1_ovf_handler (void)
{
