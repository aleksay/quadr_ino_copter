...
void
timer0_init (timer_mode _mode, uint16_t _prescaler)
{
...
void
timer0_mode_phasecorrect_top_init ()
{
}
void
timer0_mode_fastpwm_top_init ()
{
}
void
timer0_mode_fastpwm_ocra_init ()
...
void
timer0_setPin (char _pin, timer_pin_mode _mode)
{
...
void
_timer0_setPrescaler (uint16_t _prescaler)
{
...
void
timer0_setPrescaler (uint16_t _prescaler)
{
  ...
  for (i = 0; i < timer0_allowedPrescalersLenght; i++)
    {
      if (timer0_prescaler == timer0_allowedPrescalers[i])
        {
          return timer0_allowedPrescalers[(i - 1)
                                          % timer0_allowedPrescalersLenght];
        }
    }
}

uint16_t
timer0_getNextPrescaler ()
{
...
uint16_t
timer0_getRequiredPrescaler (uint32_t Hz)
{

  uint8_t i;
  for (i = 0; i < timer0_allowedPrescalersLenght; i++)
    {
      if (Hz >= prescalerMinHz (timer0_mode, timer0_allowedPrescalers[i],
                                TIMER_REGISTER_SIZE))
        {
...
timer0_resetCounter ()
{
  ...
  if (timer0_mode == CTC_OCRA || timer0_mode == FASTPWM_OCRA
      || timer0_mode == PHASE_CORRECT_OCRA)
    {
      TCNT0 = 0;
    }
}
...
timer0_start ()
{
  timer0_start (timer0_prescaler);
}

void
timer0_start (uint16_t _prescaler)
{
  if (timer0_mode == NORMAL)
    {
      timer0_resetCounter ();
...
timer0_setTop (uint16_t _top)
{
  ...
  if (timer0_mode == NORMAL || timer0_mode == FASTPWM_TOP
      || timer0_mode == PHASE_CORRECT_TOP)
    {
      timer0_top = 255;
    }
  if (timer0_mode == PHASE_CORRECT_OCRA || timer0_mode == CTC_OCRA
      || timer0_mode == FASTPWM_OCRA)
    {
...
void
timer0_setBottom (uint8_t _bottom)
{
  timer0_bottom = _bottom;
}

// from 31 Hz to 2 MHz
void
timer0_setFrequency (uint32_t Hz)
{
...
timer0_setDuty (uint8_t _duty)
{
  ...
  if (timer0_mode == PHASE_CORRECT_OCRA || timer0_mode == FASTPWM_OCRA
      || timer0_mode == CTC_OCRA)
    {
...
uint16_t
timer0_getPrescaler (void)
{
  return timer0_prescaler;
}

uint32_t
timer0_getFrequency (void)
{
  if (timer0_mode == PHASE_CORRECT_OCRA || timer0_mode == PHASE_CORRECT_TOP)
    {
...
void
timer0_register_COMPA_callback (void (*func) (void))
{
  SET_TIMER0_INTERRUPT_OUTPUTCOMPARE_A;
  timer0_compa_handler = func;
}
void
timer0_register_COMPB_callback (void (*func) (void))
{
  SET_TIMER0_INTERRUPT_OUTPUTCOMPARE_B;
  timer0_compb_handler = func;
}
void
timer0_register_OVF_callback (void (*func) (void))
{
  SET_TIMER0_INTERRUPT_OVERFLOW;
